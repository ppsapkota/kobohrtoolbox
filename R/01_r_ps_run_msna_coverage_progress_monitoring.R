...
if (hub=="TurkeyXB"){
  ...
  #####STEP 1--Export-----
  source("./R/00_1_r_ps_kobo_export_data_NES.R")
  #####STEP 2--Merge data---
  t_stamp <- format(Sys.time(),"%Y%m%d_%H%M")
  #
  xlsx_path<-"./Data/01_Download_CSV/NES/"
  save_path<-"./Data/00_Coverage/NES/"
  parter_list_f<-"./Data/MSNA2018_NES_coverage_summary_govpcode.xlsx"
}
...
d_coverage_comm<-d_merged %>% 
                 select(`Q_M/admin1`,`Q_M/admin2`,`Q_M/admin3`,`Q_M/admin4`,`Q_M/neighborho`, `Q_E/Q_E6`,Organisations)
...
data<-d_merged
#Enumerators
admin_fields<-c("Q_E/Q_E4","Q_E/Q_E5","Q_E/Q_E6","Q_M/admin1","Q_M/admin2","Q_M/admin3","Q_M/admin4","Q_M/neighborho")
...
#############----------------------Partner Name-----------------------------------------------
partner_list<-read_excel(parter_list_f, col_types = "text")
partner_list<-select(partner_list,organization_code,Organisations,Total_Communities_Planned)
...
d_num_records_partners<-data_ki %>%
                        select(organization_code, Organisations,admin4,admin5,`Q_E/Q_E4`)%>%
...
d_num_records_partners<-full_join(select(partner_list,organization_code,Organisations, Total_Communities_Planned),d_num_records_partners,by=c("organization_code"))
...
openxlsx::write.xlsx(d_num_records_partners,paste0(save_path,"msna2018_data_coverage_gap","_",t_stamp,".xlsx"),sheetName="msna2018_data_coverage_gap",row.names=FALSE)
