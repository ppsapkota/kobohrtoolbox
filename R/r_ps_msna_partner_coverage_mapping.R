...
#read Admin daa
admin4<-read_excel("./Data/Admin/syr_admin_180627.xlsx", sheet="admin4")
admin4_pop<-read_excel("./Data/Admin/admin4_pop_access.xlsx")
admin4_pop<-admin4_pop %>% 
            select(Admin4Pcode, AOI_Jun_2018 ,Population_Class,numRequired_Partners)
#merge files
xlsx_path<-paste0("./Data/00_Coverage/MSNA2018_coverage_all_partners_individual")
...
d_partner_num_communities<- d_merged %>% 
                        ...
                        summarize (numPartner=n()) %>% 
                        spread(key=admin1Name_en, val=numPartner) %>% 
                        ungroup()
...
#Coverage_gap
d_admin4_npartner$coverage_gap<-d_admin4_npartner$numRequired_Partners - d_admin4_npartner$numPartner
